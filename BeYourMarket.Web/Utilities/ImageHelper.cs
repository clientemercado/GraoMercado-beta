...
namespace BeYourMarket.Web.Utilities
{
    public class ImageHelper
    {
        ...
        {
            //var filePath = string.Format("~/images/listing/{0}.jpg", id.ToString(fileFormat));
            var filePath = (id > 0) ? string.Format("~/images/listing/{0}.jpg", id.ToString(fileFormat)) : "~/images/listing/simbol_sem_imagem.jpg";
        ...
        public static string GetLogoImagePath(string name, string format = "png", bool returnEmptyIfNotFound = false)
        {
            var filePath = string.Format("~/images/logo/{0}.png", name, format);
            if (File.Exists(HostingEnvironment.MapPath(filePath)))
            {
                return ImageVersion(filePath);
            }
